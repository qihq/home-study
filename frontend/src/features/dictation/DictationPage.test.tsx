...
it('hides spelling until parent reveals the answer and then records an incorrect score', async () => {
  ...
  const hiddenAnswer = screen.getByRole('button', { name: '显示答案' })
  expect(hiddenAnswer).toHaveClass('answer-area')
  expect(hiddenAnswer).toHaveTextContent('点击这里显示单词')
  await user.click(hiddenAnswer)
